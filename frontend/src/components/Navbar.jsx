    ...
    <div className='px-5 lg:px-40 w-full bg-[#EFFBFF]  py-8 flex justify-between items-center'>
      ...
      <div className=' gap-10 lg:flex hidden'>
        {links.map((link, index) => {
          ...
          return (
            <Link className={`${isactive ? 'border-b text-[#0088FF]' : "text-[#102D47]"}`} to={link.href} key={index}>{link.name}</Link>
          )
        })}
      </div>

    <button className='lg:hidden block' onClick={() => setIsMenuOpen(!isMenuOpen)}> {isMenuOpen ? <FaTimes /> : <BiMenu />   } </button>
    ...
    <div className={`${isMenuOpen ? "opacity-100" : "opacity-0 pointer-events-none"} z-50 absolute h-full w-[80%] bg-[#EFFBFF] top-0 left-0 bottom-0 transition-opacity duration-500 delay-75 ease-in  py-8 lg:hidden block` }>
      ...
      <div className=' flex flex-col gap-5'>
        {links.map((link, index) => {
          const isactive = location.pathname === link.href
          return (
            <Link className={`${isactive ? 'bg-[#0088FF]  text-white' : "text-[#102D47]"} py-2 px-10`} to={link.href} key={index}>{link.name}</Link>
          )
        })}
      </div>
